...
def test_predictions_to_class():
    ...
    for analysis in options:
        new_dataset = predictions_to_class(X_test, y_test, y_predicted, new_classes=analysis)
...
def test_misclass_rate_feature():
    dataset = pd.DataFrame ({"Feature 1":  [1, 2.5, 3.5, 5], "Feature 2": [1, 6, 8, 9], 
                 "Miss & Class": ["True", "False", "> Prediction hit", "True"]}, 
                 columns = ["Feature 1", "Feature 2", "Miss & Class"])
    dataset_misclass = pd.DataFrame ({"Feature 1":  [1, 2.5, 5], "Feature 2": [1, 6, 9], 
                 "Miss & Class": ["True", "False", "True"]}, 
                 columns = ["Feature 1", "Feature 2", "Miss & Class"])
    bin_number = 2
    
    result = misclass_rate_feature(dataset, dataset_misclass, feature="Feature 1", bins=bin_number)
    assert len(result) == 3
