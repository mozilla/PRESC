...
def test_split_test_train(wine_dataset_wrapper,expected_wine_dataset):
    ...
    expected_train_X, expected_test_X, expected_train_y, expected_test_y =  train_test_split(   expected_wine_dataset.iloc[:,:-1],
                                                                                                expected_wine_dataset["recommend"],
                                                                                                random_state = random_state,
                                                                                                test_size = test_size )

    expected_train = expected_train_X.merge(expected_train_y,left_index=True, right_index=True, how='left')
    expected_test = expected_test_X.merge(expected_test_y,left_index=True, right_index=True, how='left')
