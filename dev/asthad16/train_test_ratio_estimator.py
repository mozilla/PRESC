...
def split_table(x,y1):
    ...
    split_table = pd.DataFrame(np.column_stack([train_set,test_set,accuracy,f1,precision,recall]), 
                               columns=["train_set(%)","test_set(%)","accuracy","f1_score","precision_score","recall_score"])
    return(split_table)
