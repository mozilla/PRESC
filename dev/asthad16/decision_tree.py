...
def tree_model_gini(X_test,X_train,y_train,d,l):
    ...
    clf_gini = DecisionTreeClassifier(criterion = "gini",  splitter="random",random_state = 42,max_depth=d, min_samples_leaf=l)
...
def tree_model_entropy(X_test,X_train,y_train,d,l):
    ...
    clf_entropy = DecisionTreeClassifier(criterion = "entropy", random_state = 42,max_depth=d, min_samples_leaf=l)
...
def dtree_grid_search(X,y,nfolds):
    ...
    param_grid = { 'criterion':['gini','entropy'],'max_depth': range(1, 15),'min_samples_leaf' : range(2,5)}
    # decision tree model
