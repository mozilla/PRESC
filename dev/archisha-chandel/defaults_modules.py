...
def pay_amt_clstrs(data, clustering_pay_amt):
    ...
    df_name = pd.get_dummies(
        df_name, 
        columns=['pay_amt_clusters']
    )
...
def pay_clstrs(data, clustering_pay):
    ...
    filter_pay = data.filter(
        regex = r'^pay_\d{1}_-?\d',
        axis = 1
    )
...
def drop_clstrs(data):
    ...
    filter_pay_amt_clstrs = data.filter(
        regex=r'^pay_amt_clusters_\d', 
        axis=1
    )
    data.drop(
              columns = filter_pay_amt_clstrs.columns, 
              axis = 1, 
              inplace = True
             )

    # filtering and dropping bill_clusters
    filter_bill_clstrs = data.filter(
        regex=r'^bill_clusters_\d',
        axis=1
    )
    data.drop(
        columns = filter_bill_clstrs.columns,
        axis = 1,
        inplace = True
    )
