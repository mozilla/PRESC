...
def plotDistancesFromDecisionBoundary(param, preds, y_test, param_name):
    ...
    '''
    correct = [pred == true for pred, true in zip(preds, y_test)] # Which data points are misclassified
    misclassified = [] # misclassified indices
    param_mis = []  # distances or probabilities of misclassified data points from decision boundary
    correct_data = []  # correctly classifed indices
    param_cor = []  # distances or probabilities of correctly classifed data points from decision boundary
    i = 0
    ...
    plt.plot(correct_data, param_cor, 'o', color='red', alpha=.5, ms=10, label="correctly classified") # correctly classified
    plt.plot(misclassified, param_mis, 'o', color='black', alpha=.5, ms=10, label="misclassified data") # misclassified data
