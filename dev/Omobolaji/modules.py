...
def logistic_regression_model(train_features, test_features, train_target, test_target):
    ...
    cv_results = model_selection.cross_val_score(lr, train_features, train_target, cv=kfold, scoring = 'accuracy')
...
def k_neighbours_classifier_model(train_features, test_features, train_target, test_target):
    ...
    kfold = model_selection.KFold(n_splits = 10, random_state = 42)
    cv_results = model_selection.cross_val_score(knn, train_features, train_target, cv=kfold, scoring = 'accuracy')
    cv = cv_results.mean(), cv_results.std()
    print("Cross Validation: ", cv)
...
def decision_tree_classifier_model(train_features, test_features, train_target, test_target):
    ...
    kfold = model_selection.KFold(n_splits = 10, random_state = 42)
    cv_results = model_selection.cross_val_score(dtree, train_features, train_target, cv=kfold, scoring = 'accuracy')
    cv = cv_results.mean(), cv_results.std()
    print("Cross Validation: ", cv)
...
def linear_discriminant_analysis_model(train_features, test_features, train_target, test_target):
    ...
    kfold = model_selection.KFold(n_splits = 10, random_state = 42)
    cv_results = model_selection.cross_val_score(lda, train_features, train_target, cv=kfold, scoring = 'accuracy')
    cv = cv_results.mean(), cv_results.std()
    print("Cross Validation: ", cv)
...
def gaussian_nb_model(train_features, test_features, train_target, test_target):
    ...
    kfold = model_selection.KFold(n_splits = 10, random_state = 42)
    cv_results = model_selection.cross_val_score(nb, train_features, train_target, cv=kfold, scoring = 'accuracy')
    cv = cv_results.mean(), cv_results.std()
    print("Cross Validation: ", cv)
