...
def cv_space_traversal(ml_clf, data_features, data_labels, fold_num):
    ...
    for fold in k_folds:
        ...
        for train_split, test_split in rk.split(data_features, data_labels):
            ...
            train_minority, test_minority = (
                len(y_train[y_train == 1]),
                len(y_test[y_test == 1]),
            )
            train_majority, test_majority = (
            ...
            stratkfold_precision_scores.append(
                round(precision_score(y_test, y_pred), 3)
            )
            stratkfold_recall_scores.append(round(recall_score(y_test, y_pred), 3))
            stratkfold_fbeta_scores.append(
                round(fbeta_score(y_test, y_pred, beta=0.3), 3)
            )

        average_accuracy = round(
            sum(stratkfold_accuracy_scores) / len(stratkfold_accuracy_scores), 3
        )
        average_accuracies_stratkfold.append(average_accuracy)
        average_precision = round(
            sum(stratkfold_precision_scores) / len(stratkfold_precision_scores), 3
        )
        stratkfold_average_precision.append(average_precision)
        average_recall = round(
            sum(stratkfold_recall_scores) / len(stratkfold_recall_scores), 3
        )
        stratkfold_average_recall.append(average_recall)
        average_fbeta = round(
            sum(stratkfold_fbeta_scores) / len(stratkfold_fbeta_scores), 3
        )
...
def cross_val_table(num_folds, accuracy_vals, precision_vals, recall_vals, fbeta_vals):
    cross_val_data = {
        "Folds": num_folds,
        "Average Accuracy Score": accuracy_vals,
        "Average Precision Score": precision_vals,
        "Average Recall Score": recall_vals,
        "Average fbeta Score": fbeta_vals,
    }
