...
def performance_over_folds(clf, num_fold=10):
	...
	for i,fold in enumerate(folds):
		fit_times[i] = np.mean(cross_validate(clf, x, y, cv = fold, return_train_score=True)['fit_time'])
		train_scores[i] = np.mean(cross_validate(clf, x, y, cv = fold, return_train_score=True)['train_score'])
		test_scores[i] = np.mean(cross_validate(clf, x, y, cv = fold, return_train_score=True)['test_score'])
