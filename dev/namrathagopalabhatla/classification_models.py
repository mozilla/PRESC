...
class Classifier:
    ...
    def show_misclassification(self, X=None, y=None):
        ...
        for unique_class in unique_classes:
            ...
            plt.scatter(LDA[true_positives][:, 0], LDA[true_positives][:, 1], c='green', label='True Positives')
            plt.scatter(LDA[false_positives][:, 0], LDA[false_positives][:, 1], c='red', label='False Positives')
            plt.legend()
    ...
    def AUROC(self, X=None, y=None):
        ...
        if self.roc is None and self.auroc is None and (X is None or y is None):
            print('Data not provided.')
            return None
        elif self.roc is None and self.auroc is None and (X is not None and y is not None):
            _, _ = self.validate(X, y)
        
        plt.figure(figsize=(10, 10))

        for i in range(len(self.roc)):
            plt.plot(self.roc[i][0], self.roc[i][1], label='Class {} vs Rest'.format(self.roc[i][2]))
        ...
        for i in range(len(self.roc)):
            print('AUROC Score for Class {0}: {1}'.format(self.roc[i][2], self.auroc[i]))
