...
def table_descriptors(dataset):
    ...
    labels = sorted(dataset.iloc[:,-1].unique())
    print("\nThis dataset has %s classes: " % len(labels))
    print(labels)
    
    rows_with_label = []
    for label_name in labels:
        rows_with_label += [len(dataset.loc[dataset.iloc[:, -1] 
                            == label_name])]
    ...
    if imbalanced:
        print("Your dataset is imbalanced. There is at least one class "
              f"with {maximum_imbalance:.1f} times more samples than another.")
    elif maximum_imbalance >= 1.1:
        print(f"There is a class with {maximum_imbalance:.1f} times more " 
...
def labels_to_colors(dataset):
    ...
    labels_color_RGBA = [colormap((x+0.5)/len(labels)) 
...
def scaling_plot(dataset, dataset_name):
    ...
    Returns:
        None
    '''
    parameters_min_max = pd.concat([dataset.describe().iloc[3], 
                        dataset.describe().iloc[7]], axis=1, sort=False)
    ...
    parameters_min_max.loc[:, "interval"] = (parameters_min_max["max"] 
                                             -parameters_min_max["min"])
    print(parameters_min_max[::-1]) 

    # Plots the variable intervals
    fig, ax = plt.subplots()
    ax.barh(parameters_min_max.index, 
            parameters_min_max["max"]-parameters_min_max["min"], 
            left=parameters_min_max["min"], height=0.4)

    ax.grid(b=True,which="major", axis="x", markevery=slice(0, -1, 1))
    plt.tick_params(axis="y",  bottom=True, left=False, direction="in", 
                    labelleft=True, pad=20)
...
def is_number(s):
    '''Simple function that checks if the argument is a number.
    
    Parameters:
        s: The object that needs to be classified as a number or not a
            number.
    Returns:
...
def histograms(dataset, dataset_name, stacked_classes, grid=3, bins=11):
    ...
    figure, axs = plt.subplots(rows_of_histograms, row_width, 
    ...
    for subplot_index in range(row_width*rows_of_histograms):
        ...
        if subplot_index > (number_parameters-1):
            ...
            for label_name in labels:
                data_separated_labels += [dataset[(dataset[label_title] == 
                                          label_name)].iloc[:,subplot_index]]
                data_separated_labels[-1] = (
                          data_separated_labels[-1].replace({True: 1, 
                                                             False: 0}))
                                          
            # Plot stacked histograms single row
            if stacked_classes and rows_of_histograms == 1:
                axs[subplotY].hist(data_separated_labels, bins = bins, 
                    histtype = "barstacked", label=list(dataset)[subplot_index], 
                    edgecolor="black", color = labels_color_RGBA)
                if (is_number(dataset.iloc[0,subplot_index]) == True 
                    and dataset.iloc[:,subplot_index].max() > 100000):
                    axs[subplotY].ticklabel_format(axis='x', style='sci', 
                    scilimits=(-2,2))

            # Plot stacked histograms multiple rows
            elif stacked_classes and rows_of_histograms > 1:
                axs[subplotX, subplotY].hist(data_separated_labels, bins = bins, 
                    histtype = "barstacked", label=list(dataset)[subplot_index], 
                    edgecolor="black", color = labels_color_RGBA)
                if (is_number(dataset.iloc[0,subplot_index]) == True 
                    and dataset.iloc[:,subplot_index].max() > 100000):
                    axs[subplotX, subplotY].ticklabel_format(axis='x', 
                                         style='sci', scilimits=(-2,2))

            # Plot overlapping histograms single row
            elif not stacked_classes and rows_of_histograms == 1:
                axs[subplotY].hist(data_separated_labels, bins = bins, 
                    histtype = "stepfilled", label=list(dataset)[subplot_index], 
                    alpha=0.6, edgecolor="black", color = labels_color_RGBA)
                if (is_number(dataset.iloc[0,subplot_index]) == True 
                    and dataset.iloc[:,subplot_index].max() > 100000):
                    axs[subplotY].ticklabel_format(axis='x', style='sci', 
                                         scilimits=(-2,2))
            ...
            # Plot overlapping histograms multiple rows
                ...
                axs[subplotX, subplotY].hist(data_separated_labels, 
                    bins = bins, histtype = "stepfilled", 
                    label=list(dataset)[subplot_index], alpha=0.6, 
                    edgecolor="black", color = labels_color_RGBA)
                if (is_number(dataset.iloc[0,subplot_index]) == True 
                    and dataset.iloc[:,subplot_index].max() > 100):
                    axs[subplotX, subplotY].ticklabel_format(axis='x', 
                                        style='sci', scilimits=(-2,2))
            
            if rows_of_histograms == 1:
                plt.setp(axs[subplotY], 
                         xlabel = list(dataset)[subplot_index])
            else:
                plt.setp(axs[subplotX, subplotY], 
                         xlabel = list(dataset)[subplot_index])
        ...
        for label_index in range(len(labels)):
            if stacked_classes:
                label_markers += [mpl.lines.Line2D([0], [0], marker='o', 
                        color='w', label=labels[label_index], 
        ...
        if rows_of_histograms == 1:
            axs[row_width-1].legend(label_markers, labels, 
                            bbox_to_anchor=(1.6, 0.8), title="Classes")
        else:
            axs[0,row_width-1].legend(label_markers, labels, 
                            bbox_to_anchor=(1.6, 0.8), title="Classes")
        ...
        if stacked_classes and rows_of_histograms == 1:
            axs[1].set_title("Parameter histograms (stacked)", pad=20, 
                               fontsize=11)
            plt.tight_layout()
            plt.savefig("%s/%s_histograms_stacked.svg" % (dataset_name, 
                                                          dataset_name))
        elif stacked_classes and rows_of_histograms > 1:
            axs[0,1].set_title("Parameter histograms (stacked)", pad=20, 
                               fontsize=11)
            plt.tight_layout()
            plt.savefig("%s/%s_histograms_stacked.svg" % (dataset_name, 
                                                          dataset_name))
        elif not stacked_classes and rows_of_histograms == 1:
            axs[1].set_title("Parameter histograms (overlapped)", 
                               pad=20, fontsize=11)
            plt.tight_layout()
            plt.savefig("%s/%s_histograms_overlapped.svg" % (dataset_name, 
                        dataset_name))
        else:
            axs[0,1].set_title("Parameter histograms (overlapped)", 
                               pad=20, fontsize=11)
            plt.tight_layout()
            plt.savefig("%s/%s_histograms_overlapped.svg" % (dataset_name, 
                        dataset_name))            
...
def histograms_specific(dataset, dataset_name, stacked_classes, 
                        list_of_variables, grid = 3, bins=11):
    ...
    histograms(dataset_temporal, dataset_name, stacked_classes, 
               grid=grid, bins=bins)
...
def histograms_all(dataset, dataset_name, stacked_classes, grid=3, bins=11):
    ...
    for page_number in range(full_pages):
        list_temporal = [list_of_variables[(i + page_number*grid**2)] 
                         for i in initial]
        histograms_specific(dataset, dataset_name, stacked_classes, 
                            list_temporal, grid=grid, bins=bins)
...
def projections(dataset, dataset_name, grid = 3):
    ...
    if number_parameters > max_number_parameters:
        number_parameters = max_number_parameters
        print("Only the crossed projections of %s parameters at a time "
              "will be plotted."  % max_number_parameters)

    # Generate a figure with each parameter histogram as a subplot
    figure, axs = plt.subplots(number_parameters, number_parameters, 
    ...
    for columnY in range(number_parameters):
        for columnX in range(number_parameters):
            ...
            axs[columnY, columnX].scatter(dataset.iloc[:,columnX],
                    dataset.iloc[:,columnY], 
                    c = dataset[label_title].map(labels_color_dictionary))
            if dataset.iloc[:, columnX].max() > 100000:
                axs[columnY, columnX].ticklabel_format(axis='x', 
                                          style='sci', scilimits=(-2,2))
            plt.setp(axs[-1, columnX], 
                     xlabel = dataset.columns[columnX])

        if dataset.iloc[:, columnY].max() > 100000:
            axs[columnY, columnX].ticklabel_format(axis='y', 
                                          style='sci', scilimits=(-2,2))
        plt.setp(axs[columnY, 0], 
                 ylabel = dataset.columns[columnY])
        
        # Prepare markers of labels for legend
        label_markers = []
        for label_index in range(len(labels)):
            label_markers += [mpl.lines.Line2D([0], [0], marker='o', color='w', 
                              label=labels[label_index], markersize=10, 
                              markerfacecolor=labels_color_RGBA[label_index])]
    
        # Display the legend of the top right subplot.
        axs[0,number_parameters-1].legend(label_markers, labels, 
                                          title="Classes",
...
def projections_all(dataset, dataset_name, grid=2):
    ...
    for page_number in range(full_pages):
        list_temporal = [list_of_variables[(i + page_number*grid)] 
                         for i in initial]
...
def explore_test_split_ratio(dataset, dataset_name, num_test_fractions, random_tries):
    ...
    test_sizes = np.linspace(1./(num_test_fractions+1), 1., num = num_test_fractions, endpoint = False)
    ...
    for fraction in test_sizes:
        ...
        for random_number in range(random_tries):
            ...
            if counter == 0:
                ...
                print(f"{interval1} (hh:mm:ss.ss) for {counter} iteration/s. \n"
            ...
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=fraction, random_state=random_number)
    ...
    standard_deviations = np.array([score_summary[x][1] for x in range(len(score_summary))])
    ...
    axs[0].fill_between(test_sizes, averages-standard_deviations, averages+standard_deviations, color = "lavender")
    ...
    create_folder(dataset_name)
    plt.savefig(f"{dataset_name}/{dataset_name}_train-test-exploration_"
...
def main():
    ...
    list_to_plot = ([list(dataset)[1]] + [list(dataset)[2]] 
                    + [list(dataset)[0]])
    print(list_to_plot)
