...
#     train_sizes = (train_percent_float*int(len(X) - len(X)*0.2)).astype(int)
    train_sizes, train_scores, validation_scores = learning_curve(estimator = estimator, X = X, y = y, cv = cv, shuffle=True )
...
#     test_percent =100 -print('Training scores:\n\n', train_scores) train_percent
    ...
    table_of_train_test_split =  pd.concat([pd.DataFrame({'Train Percent':train_percent, 'Test Percent': test_percent}),
                                           pd.DataFrame(train_scores, columns=column_names),
                                           pd.DataFrame({'Training Scores Mean' : train_scores_mean, 
...
def visual_tain_test_split_score(table):
    ...
    plt.plot(table['Train Percent'], table['Training Scores Mean'], label = 'Training Score')
    plt.plot(table['Train Percent'], table['Testing Scores Mean'], label = 'Validation Score')
