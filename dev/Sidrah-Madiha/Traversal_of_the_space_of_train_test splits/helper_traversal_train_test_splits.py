...
def performance_evaluater_over_train_test_splits(classifier,X, y,no_of_cv=5):
    ...
    for each_size in test_sizes:
        X_train, X_test, y_train, y_test = train_data_test_data_split(X,y, test_size=each_size)
        train_data_percent_list = [int((1.0-each_size)*100)] * (no_of_cv-1)
        test_data_percent_list = [int((each_size)*100)] * (no_of_cv-1)
        percent_list_table= pd.DataFrame({'% of Training Data': train_data_percent_list, '% of Testing Data': test_data_percent_list})
        folds_metric_table = single_eval.performance_evaluater_over_folds(classifier,no_of_cv,X_train,y_train) #this returns an indexes columm which I need to remove before concat
#        folds_metric_table.reset_index()  
        unit_perecent_folds_metric= pd.concat([percent_list_table, folds_metric_table], axis=1)
        all_percent_metric_dfs.append(unit_perecent_folds_metric)

    performance_evaluatar = pd.concat(all_percent_metric_dfs , axis=0).reset_index(drop=True)
...
def visualising_performance_evaluater_over_splits(CV_data_split):
    ...
    for key , group in CV_data_split.groupby(['% of Training Data', '% of Testing Data']):

        print('\t========== For ' + str(key[1]) + '% train / ' + str(key[0])+ '% test ===========')
