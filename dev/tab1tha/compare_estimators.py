...
data_train, data_test, target_train, target_test = train_test_split(
    data, target, test_size=0.30, stratify=target, random_state=42
)
...
acc_knn, target_pred_knn = k_nn.k_nearest(
    data_train=data_train,
    target_train=target_train,
    data_test=data_test,
    target_test=target_test,
)
acc_svc, target_pred_svc = s_v_m.s_vee_c(
...
plt.subplot(2, 1, 1)
explore_datav2.confu_matrix_display(conf_matrix_knn, target, "KNN")
plt.subplot(2, 1, 2)
explore_datav2.confu_matrix_display(conf_matrix_svc, target, "SVC")
plt.show()

# Create a dataframe that contains true values and the corresponding predicted values per model
